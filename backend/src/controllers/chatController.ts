...
export async function getChats(req: AuthRequest, res: Response, next:NextFunction) {
    try {
        ...
        const chats = await Chat.find({ participants: userId }).populate("participants", "name email avatar").populate("latestMessage").sort({ lastMessageAt: -1 });
...
export async function getOrCreateChat(req: AuthRequest, res: Response, next:NextFunction) {
    try {
        ...
        }).populate("participants", "name email avatar").populate("latestMessage");
